...
import { WALL, clamp, overlaps, uid } from "./plan";
 ...
 * transform list reads outside-in: scale the drawing to this item's real size,
 * spin it about its own centre, then move it into place.
 */
export function artTransform(it) {
  ...
  return `translate(${tx} ${ty}) rotate(${it.rot} ${it.w / 2} ${it.d / 2}) scale(${sx} ${sy})`;
}

/* ------------------------------------------------------------------- fit */

/** The walkable inside of a room box: the floor minus half a wall all round. */
function innerParts(rooms) {
  ...
  for (const p of boxes) {
    for (let y = p.y + WALL / 2; y + f.d <= p.y + p.d - WALL / 2 + 0.5; y += STEP) {
      for (let x = p.x + WALL / 2; x + f.w <= p.x + p.w - WALL / 2 + 0.5; x += STEP) {
        ...
        if (onFloor(rect, rooms) && !hitsFurniture(rect, items)) return { x: rect.x, y: rect.y };
      }
    }
  }
  return null;
